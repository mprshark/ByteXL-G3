...
.hero p { font-size: 1.1rem; font-weight: 700; margin-top: 12px; color: #111; }
...
@keyframes road-scroll { from { left: 110%; } to { left: -10%; } }
.car-svg {
    position: absolute;
    bottom: 10px;
    left: 40%;
...
.neo-card:hover { transform: translate(-4px,-4px); box-shadow: 12px 12px 0 #000; }
...
.metric-box .val { font-size: 2.2rem; font-weight: 900; }
.metric-box .lbl { font-size: 0.8rem; font-weight: 700; color: #555; letter-spacing: 1px; }
...
.stButton > button:active { transform: translate(4px,4px); box-shadow: 2px 2px 0 #000 !important; }
...
.price-display { font-size: 3.5rem; font-weight: 900; color: #000; line-height: 1; margin: 10px 0; }
...
<div class="hero">
  ...
  <p>Powered by XGBoost &nbsp;|&nbsp; Full ML Pipeline &nbsp;|&nbsp; RAG Insights &nbsp;|&nbsp; Group 5</p>
</div>
""", unsafe_allow_html=True)
...
def load_and_process():
    import os
    # Support all possible filenames
    for name in ["data.csv", "car_data.csv", "car data.csv"]:
        if os.path.exists(name):
            df_raw = pd.read_csv(name)
            break
...
tab1, tab2, tab3, tab4 = st.tabs([
    "Price Distribution", "Correlation Heatmap", "Fuel vs Price", "Kms vs Price"
])
sns.set_theme(style="whitegrid")

with tab1:
    fig, ax = plt.subplots(figsize=(8, 4))
    sns.histplot(df['Selling_Price'], kde=True, ax=ax, color='#FF2D87')
    ax.set_title("Selling Price Distribution", fontweight='bold')
    st.pyplot(fig); plt.close(fig)

with tab2:
    fig, ax = plt.subplots(figsize=(8, 5))
    corr = df[['Selling_Price','Present_Price','Kms_Driven','Age','Depreciation','Owner']].corr()
    sns.heatmap(corr, annot=True, fmt=".2f", cmap="YlOrRd", ax=ax, linewidths=2)
...
fe1.markdown("**Age** = 2024 - Year\n\nHow old the car is.")
fe2.markdown("**Depreciation** = (Present - Selling) / Age\n\nAnnual value loss rate.")
fe3.markdown("**Brand** = First word of Car_Name\n\nGroups by manufacturer.")
st.markdown('</div>', unsafe_allow_html=True)
...
def build_rag(_df):
    try:
        ...
        insights = (
            [f"Cars with {f} fuel type average Rs.{_df[_df['Fuel_Type']==f]['Selling_Price'].mean():.2f}L."
             for f in _df['Fuel_Type'].unique()]
            + [f"{t} transmission cars average Rs.{_df[_df['Transmission']==t]['Selling_Price'].mean():.2f}L."
               for t in _df['Transmission'].unique()]
            + [
        ...
        return insights, index, embed_model
    except Exception:
...
if rag_insights:
    ...
    st.warning("RAG fallback mode (faiss-cpu / sentence-transformers unavailable).")
st.markdown('</div>', unsafe_allow_html=True)


# ─────────────────────────────────────────────
# STEP 5 — MODEL TRAINING & ACCURACY
# ─────────────────────────────────────────────
st.markdown('<div class="section-tag">STEP 5 — MODEL TRAINING & ACCURACY</div>', unsafe_allow_html=True)

@st.cache_resource(show_spinner=False)
def train_model(_df):
    X = _df[['Present_Price','Kms_Driven','Fuel_Type','Seller_Type','Transmission','Owner','Age']]
    y = _df['Selling_Price']
...
for col_ui, label, fmt in [
    (m1, "R2 SCORE",   f"{metrics['r2']:.2%}"),
    (m2, "MAE",        f"Rs.{metrics['mae']:.3f}L"),
    (m3, "RMSE",       f"Rs.{metrics['rmse']:.3f}L"),
    (m4, "TRAIN SIZE", str(metrics['train_size'])),
    (m5, "TEST SIZE",  str(metrics['test_size'])),
]:
    col_ui.markdown(f"""
    <div class="metric-box"><div class="val">{fmt}</div><div class="lbl">{label}</div></div>
    """, unsafe_allow_html=True)

fig, ax = plt.subplots(figsize=(6, 3.5))
ax.scatter(metrics['y_test'], metrics['y_pred'], alpha=0.6, color='#FF2D87', edgecolors='#000', linewidths=0.5)
ax.plot([metrics['y_test'].min(), metrics['y_test'].max()],
        [metrics['y_test'].min(), metrics['y_test'].max()], 'k--', linewidth=2)
...
with left:
    ...
    with st.form("pred_form"):
        ...
        kms           = st.number_input("Kms Driven",             0, 500000, 25000, step=1000)
...
with right:
    if submitted:
        ...
        input_df = pd.DataFrame([{
            'Present_Price': present_price, 'Kms_Driven': kms, 'Owner': owner,
            'Age': age_val, 'Fuel_Type': fuel, 'Seller_Type': seller, 'Transmission': trans,
        }])
        prediction = max(0.0, float(model.predict(input_df)[0]))

        insight_text = "Maintain service records and keep mileage low to maximize resale value."
        if rag_insights and rag_index and rag_embed_model:
            qv = rag_embed_model.encode([f"{fuel} {trans} car"]).astype('float32')
            _, idx = rag_index.search(qv, 1)
            insight_text = rag_insights[idx[0][0]]

        st.markdown(f"""
        <div class="neo-card green">
          <div class="step">ESTIMATED PRICE</div>
          <div class="price-display">Rs. {prediction:.2f} L</div>
          <p style="font-weight:700;color:#333">Age: {age_val} yrs &nbsp;|&nbsp; {fuel} &nbsp;|&nbsp; {trans}</p>
          <div class="rag-box">
